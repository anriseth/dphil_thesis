...
attempt to always move in directions that decrease the value of
...
quasi-Newton methods, or systematically cover the search directions,
as nonlinear conjugate gradient methods do.  Further information on
iterative algorithms and line search procedures is covered in
...
modification to N-GMRES, which significantly improve sthe performance
...
Optim~\citet{mogensen2018optim} as part of the thesis work. In this
...
problems as the inner optimiser of the box-constrained optimisation
procedure \texttt{Fminbox} in Optim.  The work which this chapter is
based on has been submitted to the ``Numerical Linear Algebra with
Applications''.\todo{Update with publication} It originally came about
from an investigation of the nonlinear generalised minimal residual
method (N-GMRES) for nonlinear solvers in reservoir
...
\begin{algorithm}[htb]
  ...
  \begin{algorithmic}[1]
    ...
    \left(\vect{X}-\vect{X}^P\right)$
    \hfill {\footnotesize \Cref{sec:num_experiments}: $A\gets\Transpose{{(\vect{X}-\vect{X}^P)}}\left(
        \Grad(\vect{X})-\Grad(\vect{X}^P)
      \right)$}
...
\Cref{subsec:perf_prof}, whilst the Appendix provides tables of
...
and G, as can be seen from \Cref{tbl:probEG} in the Appendix.  One of
...
\Cref{tbl:cutest_table1,tbl:cutest_table2} of the appendix together

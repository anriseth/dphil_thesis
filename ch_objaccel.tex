...
minimiser.  We aim to find a local minimum of the optimisation problem
\begin{equation}
  \min_{x\in\mathbb{R}^n}\Obj(x).
\end{equation}
...
\begin{equation}
  \min_{\alpha\in\mathbb{R}^k}\Bigl\|\Grad(x^P)+
  \sum_{j=l}^k\alpha_l(\Grad(x^{(l)})-\Grad(x^P))\Bigr\|_2.
\end{equation}
...
\begin{align}\label{eq:okrylov_2}
  \OKrylov{k}{x}
  &=
    \mbox{span}\{x-x^{(k)},x^{(k)}-x^{(k-1)},\dots,x^{(2)}-x^{(1)}\},\\
    % &\textnormal{and}\\
  x+\OKrylov{k}{x}
  &= x^{(1)} +
    \mbox{span}\{x-x^{(k)},x^{(k)}-x^{(k-1)},\dots,x^{(2)}-x^{(1)}\}.
...
Each of the problems was tested with different sizes $n$, and for
...
\begin{table}[hp]
  ...
  \caption[Quantiles reporting $\Obj$ evaluations to reach
    ...
    tolerance for each solver on test problems A--C from
    \Cref{subsec:test_problems_sterck}. Grey rows highlight the solver with the
    best \num{0.5} quantile.
...
\begin{table}[p]
  ...
  \caption[Quantiles reporting $\Obj$ evaluations to reach
    ...
    tolerance for each solver on test problem D from
    \Cref{subsec:test_problems_sterck}. Grey rows highlight the solver with the
    best \num{0.5} quantile.
...
\begin{table}[p]
  ...
  \caption[Quantiles reporting $\Obj$ evaluations to reach
    ...
    tolerance for each solver on test problems E--G from
    \Cref{subsec:test_problems_sterck}. Grey rows highlight the solver
    with the best \num{0.5} quantile.

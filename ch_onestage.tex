...
\item an ordering between all
  decisions induced by their corresponding outcomes.
\end{itemize}
...
as the optimisation problem\footnote{Maximisation problems can
  ...
  objective, $\inf_{x\in\mathcal{X}}-f(x)$. We change between the two
  depending on the application.}
\begin{equation}\label{eq:deterministic_setting}
  \sup_{x\in\mathcal{X}} f(x).
\end{equation}
The set $\mathcal{X}$ may, for example, be a subset of the real line, a
finite-dimensional vector space, or a set of functions.
If $f$ attains its maximum on in the set $\mathcal{X}$, we can use the
notation $\max_{x\in\mathcal{X}}f(x)$.
...
In
\Cref{sec:one_optim_random_outcomes,sec:one_comparison_orderings,sec:one_randomness_constraints} we
discuss common ways to model decision makers who face
non-deterministic outcomes modelled with random variables, and
in \Cref{sec:one_multi-objective} we summarise the theory of multi-objective
optimisation.
...
been non-dimensionalised.\footnote{We illustrate the modelling and
  ...
  in \Cref{ch:discrete_control,ch:cts_control}.}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      colormap/viridis,
      % colorbar,
      ztick = {0.3,0.5,0.7,0.9},
      xtick = {0.2,0.5,0.8,1.1},
      ytick = {0.2,0.5,0.8,1.1},
      xmajorgrids={true}, ymajorgrids={true}, zmajorgrids={true},
      ]
      \addplot3[surf]
      {exp(-2*x)*(1-exp(-2*y/x)) +
        0.9*exp(-1.8*y)*(1-exp(-0.8*x/y))*(1-exp(-8*1/y))
        + 1.2*exp(-2*1)*(1-exp(-3*x/1))
      };
    \end{axis}
  \end{tikzpicture}
...
\begin{align}\label{eq:mean_cov_y_example}
  m_Y
...
we model using a random variable $Y$,
...
``rational'' decision maker, exemplified with the so-called Borch
paradox.
A suggested remedy looks at modelling the
...
where each value $y\in\mathcal{Y}$ requires the solution to partial
...
None of these references discusses to what extent super-quantiles change the
...
Fix the distribution of a random variable $Y$. If $f(x,Y)$ is normally distributed for each value of
...
indicates that the approaches above implicitly define a
...
\begin{example}[Pricing problem]
  ...
  The mean $m_Y$ and covariance matrix $C_Y$ of $Y$ are defined
  in~\eqref{eq:mean_cov_y_example}.
  ...
  \begin{figure}[htbp]
    ...
    \caption[Comparison between two decision maker models]{The left
      ...
      The drop in the value of the left plot relates is due to
      projecting the three-dimensional difference vector in
      $\mathcal{X}$ down to one dimension.
...
\begin{example}[Acceptance sets]
  For $Y$ a random variable taking values in $\mathcal{Y}$,
...
optimal in the Pareto sense. \Cref{fig:multi_map} and
\Cref{fig:pareto_local_disconnected} provide visual representations of
the multi-objective concepts for two-dimensional problems
...
\begin{figure}[htbp]
  ...
  \end{tikzpicture}
  \caption[Pareto concepts for two-dimensional problem]{Pareto concepts for a mapping $F(x)=(f_1(x),f_2(x))$.
...
true. \Cref{fig:pareto_local_disconnected}
    ...
    \begin{subfigure}[h]{.5\textwidth}
      \begin{tikzpicture}
        \begin{axis}[width=0.95\textwidth,
          ...
          scaled ticks = false,
          xtick = {0, 0.003, 0.006, 0.009},
          xticklabels = {0, 0.003, 0.006, 0.009},
          ytick = {0,0.04,0.08,0.12},
          yticklabels = {0,0.04,0.08,0.12},
          ]
          \addplot[thick, name path=A] table {./data/pareto_std_prof_3_nbi.dat};
          \path [name path=B]
          (0,0) -- (0.010315778171513777, 0);
    ...
    \begin{subfigure}[t]{.5\textwidth}
      \begin{tikzpicture}
        \begin{axis}[width=0.9\textwidth,
          ...
          \addplot [samples=50,name path=A,domain=0:5,blue,draw opacity=0.1]
          {5*exp(-x)+2*exp(-0.5*(x-3)^2)};
          \path [name path=B]
          (0,5.022) -- (5,5.022);
          \addplot [blue, fill opacity = 0.1] fill between [of=A and B, soft clip={domain=0:5}];

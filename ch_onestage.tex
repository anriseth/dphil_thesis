...
uncertainty in $f(x,\cdot)$, or, if the distribution is bimodal, the
...
\begin{problem}[Mean-deviation]
  ...
  \begin{align}
    \max_{x\in\mathcal{X}}\{(\mathbb{E}[f(x,\cdot)],\,-\mathbb{D}[f(x,\cdot)])\}.
  \end{align}
\end{problem}
...
\begin{example}[Nonlinear expectations]
  ...
  E[e^{-\lambda X}]$, is called an entropic nonlinear expectation,
  with $\lambda>0$.
...
\begin{example}[Pricing problem]
  ...
  Assume that the model for product demand $q(x)=(q_1(x),q_2(x))$ for the two products over
  this period is
  ...
  \end{align}
  respectively.\todo{Note non-dimensionalised model}
...
\begin{example}[Two-dimensional Pareto front]
  ...
  \end{figure}
\end{example}
\todo[inline]{Picture of weak Pareto optimal}
...
\begin{example}[Retail application]\label{ex:retail_pareto}
  ...
  Assume that the model for product demand $q(x)=(q_1(x),q_2(x),q_3(x))$ for the
  ...
  In what follows, the particular values of $\alpha$, $\beta$, $m_y$, and $C_y$ are
  \begin{align}
    \alpha

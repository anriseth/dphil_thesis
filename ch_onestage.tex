...
We will present a toy-problem in pricing three retail products, in
...
$x=(x_1,x_2,x_3)$
of the products in order to improve the profit or revenue for the next
time period.
...
for the three products over the next period is
...
\begin{figure}[htbp]
  ...
  \caption{The sum $q_1(x)+q_2(x)$ shows the demand for products one
    and two with $x_3=1$.
    The model is unlikely to be valid when $x_1=x_2=0$, as it predicts
    zero demand for free products.}\label{fig:total_volume_2d}
\end{figure}
...
\begin{align}
  ...
  \end{pmatrix}
           \times 10^{-3}.
\end{align}
\Cref{fig:example_profit_distributions} show

\begin{figure}[htb]
  \centering
  \todo[inline]{Plot profit for two or three prices}
  \caption{The probability distribution of profit for three
    combinations of prices.}\label{fig:example_profit_distributions}
\end{figure}
...
we model using an as a random variable in $\mathcal{Y}$.
...
uncertainty in $f(x,y)$, or, if the distribution is bimodal, the
...
\begin{example}[Deviation measures]
  ...
  \begin{align}
    X\mapsto \mathbb{E}[\min(\mathbb{E}[X]-X,0)].
...
small changes in $\lambda$.\todo{Reference example} Small errors in determining the best
...
\todo[inline]{Define sublinear expectation}
...
\begin{example}[Nonlinear expectations]
  ...
  optimization \citep{ben2009robust}. If there is an attached
...
deviation. Then the mean-deviation formulation may then be preferable in
practical settings.
...
\begin{example}[Pricing problem]
  \todo[inline]{Smooth out transition}
...
Introduce a deterministic, objective $g:\mathcal
...
some neighbourhood around $x$, see.\todo{reference fig}
...
\begin{example}[Two-dimensional Pareto front]
  ...
  multi-objective concepts for a two-dimensional problem
...
\begin{mydef}
  ...
  The utopia point $\underline{F}=(\underline{f_1},\dots,\underline{f_m})$
...
\begin{example}[Two-dimensional bounding box]
  \Cref{fig:pareto_drawing} illustrates an example of the utopia and
  nadir points $\underline{F}$ and $\overline{F}$,
  together with the bounding box for a two-dimensional problem $F:\mathcal{X}\to\mathbb{R}^2$.
  \begin{figure}[htbp]
    \centering
    ...
    \caption{An example Pareto front $\mathcal{P}$ of two objectives. The utopia and
      nadir points $\underline{F}$ and $\overline{F}$ represent the
...
\begin{example}[Comparison of Pareto front
  ...
  locally, but not globally, Pareto optimal.\todo{plot this as example
    above and reword this sentence}
  ...
  In \Cref{fig:pareto_disconnected} one can see how the three
    ...
    \begin{subfigure}[t]{.5\textwidth}
      \begin{tikzpicture}
        \begin{axis}[width=0.9\textwidth,
          ...
          \addplot[only marks, mark size=0.5] table {./data/pareto_disconnected_nbi_eq.dat};
        \end{axis}
      \end{tikzpicture}
...
\begin{subfigure}[t]{.5\textwidth}
      \begin{tikzpicture}
        ...
        \end{axis}
      \end{tikzpicture}
      \caption{Varying constraint on $f_1$}
    \end{subfigure}\\[1em]
    \begin{subfigure}[t]{.5\textwidth}
      \begin{tikzpicture}
        \begin{axis}[width=0.9\textwidth,
    ...
    \end{subfigure}
    \todo[inline]{Add Pareto front curve}
    \caption{Approximations of a disconnected Pareto front.

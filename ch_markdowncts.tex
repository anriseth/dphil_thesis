...
The chapter is structured as follows: In \Cref{sec:modelling}, we
...
suggest avenues for further research in \Cref{sec:conclusion}.
...
\begin{example}\label{ex:acecplot}
  ...
  \Cref{fig:hjbsol_a_cec} shows a plot of $a^D(t,s,1.0)$ for a given set of
...
\begin{example}\label{ex:function_impact_uncertainty}
  ...
  Example~\ref{ex:acecplot}. %\Cref{ex:acecplot}.
...
\begin{figure}[htb]
  ...
  \caption{Statistics of the price paths from
    Example~\ref{ex:impact_uncertainty}, %\Cref{ex:impact_uncertainty},
...
% \end{equation}
% The terminal condition for this function is $e(1,s,\theta,r)=r-Cs$.
...
% by~\cite{pratt1964risk}. For twice differentiable utility functions,
...
%   certainty equivalent value if $\mathcal O(\mathcal X(e))=\mathcal
%   O\left(\rho\left( {\Sigma(t,\theta)}^{-1}
...
%   $R^\alpha(1)=r+\int_t^{T_h}q(\alpha(u))\alpha(u)\,\mathrm{d}u$.
%   Second, note that
%   $U(R^\alpha(1)-CS^\alpha(1)) =  e^{-\gamma r}
%   U\left( \int_t^{T_h}q(\alpha(u))\alpha(u)\,\mathrm{d}u-CS^\alpha(1) \right)$.
%   The certainty equivalent value function that satisfies the
%   PDE~\eqref{eq:hjb_ceutility} can be written as
%   \begin{align}
%     e(t,s,\theta,r)
%     &=-\frac{1}{\gamma} \log
%     \left\{ -\max_{\alpha \in\mathcal A} \mathbb E_t
%       \left[ U\left( R^\alpha(1)-CS^\alpha(1) \right)\right]\right\} \\
%     &= r - \frac{1}{\gamma}\log
%     \left\{ -\max_{\alpha \in\mathcal A} \mathbb E_t
%       \left[U\left( \int_t^{T_h}q(\alpha(u))\alpha(u)\,\mathrm{d}u-CS^\alpha(1) \right)\right]\right\}.
%   \end{align}
%   There is no explicit dependence on $r$ in the maximization term, and
...
Result~\ref{res:linear_asymptotics} in \Cref{sec:stochastic_hjb}

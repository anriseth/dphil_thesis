...
\begin{align}
  \hat{G}(0)&=1,\\ &= \hat{W}(\hat{t}) / \sqrt{\hat{T}}
  \mathbb{E}[\hat{G}(\hat{t}+\Delta{\hat{t}})\mid \hat{G}(\hat{t})]&=\hat{G}(\hat{t}),
...
In the limit $t\to T$, we see from~\eqref{eq:def_value_interior} that $v(T,s,g)=-Cs$.
In the usual way, see for example \citet{pham2009continuous}, the HJB
equation for $v$ is
